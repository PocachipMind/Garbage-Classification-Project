...
def visualize_augmentations(dataset, index=0, samples=20, columns=5):
    ...
    dataset.transforms = A.Compose(
        [transform for transform in dataset.transforms if not isinstance(transform, (A.Normalize, ToTensorV2))])
...
def train(train_loader, valid_loader, epoch_num, model, device, criterion, optimizer, scheduler):
    ...
    accuracy_df = pd.DataFrame(index=list(range(epoch_num)),
    ...
    for epoch in range(1, epoch_num + 1):
        ...
        print(
            'Epoch: [{:3d} / {:3d}]   Train Loss: {:.3f}   Train Accuracy: {:.3f} '.format(
...
def test(test_loader, device, label_quantity):
    ...
    model.load_state_dict(torch.load('./DenseNet_best.pt', map_location=device))
    model.to(device)
